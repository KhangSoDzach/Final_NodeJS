...
// Kiểm tra nếu không có dữ liệu user, khởi tạo dữ liệu admin
...
// Kiểm tra nếu không có dữ liệu product, khởi tạo dữ liệu sản phẩm mẫu
if (db.products.countDocuments() === 0) {
  print("Khởi tạo dữ liệu sản phẩm mẫu...");
  ...
  print(`${demoProducts.length} sản phẩm mẫu đã được tạo!`);
}

// Kiểm tra nếu không có dữ liệu mã giảm giá, khởi tạo coupons
if (db.coupons.countDocuments() === 0) {
  print("Khởi tạo dữ liệu mã giảm giá...");
  ...
  db.coupons.insertMany(demoCoupons);
  print(`${demoCoupons.length} mã giảm giá đã được tạo!`);
}
